...
export default function PostFeed() {
  ...
  const fetchPosts = async () => {
    try {
      ...
      // Normalizar reacciones y comentarios para evitar errores
      const normalizedPosts = data.map((post: any) => ({
        ...post,
        reacciones: Array.isArray(post.reacciones) ? post.reacciones : [],
  ...
  const handleLike = (postId: string) => {
    setPosts(posts.map(post =>
      post.id === postId ? { ...post, reacciones: [...post.reacciones, {}] } : post
    ))
  }
        ...
        {posts.map((post) => (
          <div key={post.id} className="bg-black/60 border border-green-500/30 rounded-lg p-4">
            ...
            <div className="flex justify-between items-start mb-3">
              <div className="flex gap-3">
                ...
                <div>
                  <div className="flex items-center gap-1">
                    <span className="font-bold text-white">{post.autor.username}</span>
                  </div>
              ...
              <DropdownMenu>
                <DropdownMenuTrigger asChild>
                  <Button variant="ghost" size="icon" className="rounded-full text-gray-400 hover:bg-green-500/20 hover:text-white">
            ...
            <div className="flex justify-between text-gray-400 border-t border-green-500/20 pt-3">
              <Button variant="ghost" size="sm" className="flex items-center gap-1 hover:text-green-400 hover:bg-green-500/10" onClick={() => handleLike(post.id)}>
