...
export async function POST(request: Request) {
  try {
    ...
    const userToDelete = body.followerId;
    ...
    console.log('targetUserId:', userToDelete);
    ...
    const backendResponse = await fetch(
      `${process.env.NEXT_PUBLIC_BACKEND_URL}/followers/deleteFollower/${userId}`,
      {
        method: 'POST',
    ...
    if (!backendResponse.ok) {
      console.log('Error en la respuesta del backend:', backendResponse);
